...
import type { Config, DarkModeConfig } from "tailwindcss/types/config";
...
const plugin = () => {
  return ({ addBase, theme }) => {
    addBase({
      ...
      h2: { fontSize: theme("fontSize.xl"), margin: 0 },
      h3: { fontSize: theme("fontSize.lg"), margin: 0 },
      p: { margin: 0 },
    });
  };
};

const config: (options: RadixThemePluginOptions) => Partial<Config> = ({
  darkMode = "selector",
  ...
  function allRadixColors() {
    ...
    [
      ...
      accentColorNames.push(
        ...group.values.filter((color) => color !== "gray"),
      );
    });
...
export const radixThemePlugin = withOptions(plugin, config);
